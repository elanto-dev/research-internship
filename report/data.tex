...
\begin{table}[h]
    ...
    \begin{tabular}{|l|c|c|c|c|c|c|c||c|}
        ...
        \hline
        Combined NE & 861 & 990 & 89 & 672 & 7980 & 8 & 760 & 11360 \\
        \hline
    \end{tabular}
    }
...
The total number of conversations shared across all the sources after duplicates and non-English conversations were removed is 11360. However, after removing duplicate conversations from the entire combined dataset, it leaves us with 10175 conversations, which means that 1185 developer-\acrshort{gpt} conversations are shared in multiple locations.

\section{Data cleaning}
During our initial analysis of the dataset, we identified the presence of information irrelevant to the research question --- referred to as ``noise''. First, many prompts contain embedded code alongside natural language. A good example of this is the following prompt from the DevGPT dataset sourced from GitHub Issues: 
...
To identify whether a line is written in a programming language or natural language, we implement a function that evaluates the line’s structures, whether they are tokens or full sentences, and assigns them a probability score: \texttt{0} indicates a programming language structure, \texttt{0.5} indicates ambiguity (could be either), and \texttt{1} indicates natural language. The list below shows what kinds of structures are considered in this script:
\begin{itemize}
...
To determine whether a line is written in natural language or code, the script calculates the average likelihood score by summing the individual scores of all structures (tokens or sentences) on the line and dividing by the total number of structures. If the resulting average is above 0.5, the line is considered to contain natural language; otherwise, it is classified as a code line.
...
As part of the data exploration process, we begin by extracting key information about the dataset. For the purposes of this study, we focus exclusively on the user-generated prompts and disregard the responses produced by \acrshort{gpt} across all conversations. 

\subsection{Number of prompts in conversations}
...
\hline
GitHub Commits       & 3.2   & 1     & 5.65               \\
\hline
GitHub Issues        & 4.57  & 2     & 8.9                \\
\hline
GitHub Discussions   & 4.66  & 2     & 9.52               \\
\hline
GitHub Pull requests & 3.36  & 2     & 7.94               \\
\hline
GitHub code files    & 6.72  & 2     & 16.74              \\
\hline
GitHub repositories  & 21.38 & 10.5  & 31.58              \\
\hline
HackerNews       & 4     & 1     & 9.12               \\
\hline 
\end{tabular}   
...
Figure~\ref{fig:conv-per-prompt} contains histograms that show how many conversations contain a certain number of prompts, where the conversations with the maximum number of 30 prompts are shown. All the conversations that exceeded the number of 30 prompts were separated into separate JSON files and their content was investigated and described below. 
...
To understand the reasons behind unusually long conversations, we manually reviewed 295 conversations that exceeded a predefined length threshold. The most common patterns are summarized below:
...
\begin{itemize}
    ...
    \item \textbf{Iterative debugging:} Users refine requests or see help with errors after initial code responses did not meet their expectations.
    \item \textbf{Programming assistance:} Users guide \acrshort{gpt} through multi-step coding tasks, testing and iterating on output.
    \item \textbf{Mentorship:} Users share their own code for feedback, explanations, or improvement suggestions.
    \item \textbf{General discussion:} Conversations focus on non-programming topics such as advice, opinions, or general discussions.
\end{itemize}

\subsection{Average symbol/word count per prompt}
The average number of symbols per prompt varied across sources, as shown in Figure~\ref{fig:symbols-per-prompt}. The \textit{x}-axis represents the prompt position within a conversation, while the \textit{y}-axis shows the average symbol count for that position. Since longer conversations are less common, later prompt positions often reflect fewer data points, leading to the prompt average pattern changes that can be visibly observed in the conversations sourced from GitHub code files or Hacker News. Across the majority of the sources, initial prompts typically have higher symbol counts, as they often introduce the problem in detail.
...
Conversations containing unusually long prompts were saved and manually reviewed to understand their content. In total, 1984 conversations contained long prompts. We identified several common reasons for the prompt length:
\begin{itemize}
    \item \textbf{Pasted external content:} Users copy large texts from sources such as scientific articles, GitHub issues, or documentation, often asking for summaries, explanations, or audience-specific rewrites.
    \item \textbf{Code-related debugging:} Prompts include error messages, undocumented code, or comments not flagged as code, with users seeking help with debugging or improvement.
    \item \textbf{Detailed problem descriptions: } Some users provide extensive background to frame their query, including:
    \begin{itemize}
...
The dataset used for this study offers a useful overview of how developers utilize \acrshort{gpt} to address everyday challenges. However, the data sources are incomplete. Conversations were collected from GitHub (commits, code files, discussions, pull requests and issues) and HackerNews, covering conversations that happened before March 2025. As a result, the dataset leaves out other platforms that may contain additional insights into developers' use of \acrshort{gpt}. Moreover, the data spans nearly two years, and some older conversation links that were used for scraping the data are now inactive, making it impossible to verify the original content.

The language detection and data cleaning tools used were also imperfect. They occasionally failed to identify prompts or prompt lines containing foreign languages or embedded code. Many prompts contain pasted content (articles, documentation, code comments, or error messages), which, although written in the natural English language, lacked relevance to the research questions. These elements introduced noise that the current toolset could not fully filter out.